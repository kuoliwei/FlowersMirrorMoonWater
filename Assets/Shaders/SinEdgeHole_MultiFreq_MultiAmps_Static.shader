Shader "Custom/SinEdgeHole_MultiFreq_MultiAmps_Static"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                //float resultAlpha = prevAlpha + (alphaMask - prevAlpha) * _MaskBlend;  // 漸進靠近目標值
                //float resultAlpha = lerp(prevAlpha, alphaMask, _MaskBlend);
                // 或簡寫為：float resultAlpha = lerp(prevAlpha, alphaMask, _MaskBlend);
                // float t = clamp( 1.0 - abs(alphaMask - prevAlpha), 0.1, 1.0);
                // float resultAlpha = lerp(prevAlpha, alphaMask, pow(t, 2.0));
